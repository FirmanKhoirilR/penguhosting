...
const CardFitureOffer = ({ item }: any) => {
  return (
    <div className="flex justify-center md:flex-row  flex-col my-8 gap-2 md:gap-10 items-center" key={item.title}>
      <motion.img whileInView={{ x: [-290, 0], opacity: [0, 1] }} transition={{ duration: 0.8 }} src={item.image} alt={item.title} className="md:h-[250px] w-[300px] h-[180px] md:w-[400px]" />
      <motion.div whileInView={{ x: [300, 0], opacity: [0, 1] }} transition={{ duration: 0.8 }} className=" min-w-[350px] p-4  max-w-[650px] flex flex-col items-start gap-4">
