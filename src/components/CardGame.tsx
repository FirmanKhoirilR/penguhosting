...
const CardGame = ({ item }: ICard) => {
  return (
    <Link to={item.url} key={item.name} className="group relative shadow-[0px_8px_12px_2px_rgb(0,0,0,0.6)] flex flex-col items-center gap-2 md:w-[240px] w-full h-[250px] overflow-hidden rounded-sm">
      <img src={item.image} alt={item.name} width={240} height={240} className="h-[250px] group-hover:scale-125 transition duration-1000 w-full object-fill md:w-[240px] ease-in-out" />
